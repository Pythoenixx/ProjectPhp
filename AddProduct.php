...
if (isset($_POST['submitted'])) {
    ...
    if (empty($_POST['supplier_id'])) {
        ...
        if (!preg_match('/^8\d{3}$/', $supplier_id)) {
            ...
            $query = "SELECT supplier_id FROM products WHERE supplier_id = '$supplier_id'";
            $result = mysqli_query($dbc, $query);
            if (mysqli_num_rows($result) > 0) {
                $errors[] = 'Supplier ID already exists.';
            }
        }
    }
    ...
    if (empty($errors)) { // If everything's okay.
        ...
        /*$product_name = mysqli_real_escape_string($dbc, $product_name);
        $product_description = mysqli_real_escape_string($dbc, $product_description);
        $cost = mysqli_real_escape_string($dbc, $cost);
        $price = mysqli_real_escape_string($dbc, $price);
        $quantity = mysqli_real_escape_string($dbc, $quantity);
        $supplier_id = mysqli_real_escape_string($dbc, $supplier_id);*/

        // Check if supplier ID already exists (in case of concurrent requests).
        $query = "SELECT supplier_id FROM products WHERE supplier_id = '$supplier_id'";
        $result = mysqli_query($dbc, $query);
        if (mysqli_num_rows($result) > 0) {
            $errors[] = 'Supplier ID already exists.';
...
<form action="AddProduct.php" method="post">
    ...
    <p>Supplier ID: <input type="text" name="supplier_id" size="5" maxlength="4" value="<?php if (isset($_POST['supplier_id'])) echo $_POST['supplier_id']; ?>" /></p>
